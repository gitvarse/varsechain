...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/VarsechainParams"
else
    PARAMS_DIR="$HOME/.varsechain-params"
fi
...
DOWNLOAD_URL="https://github.com/gitvarse/varsechain/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with VARSE_DISABLE_SOMETHING=1
VARSE_DISABLE_WGET="${VARSE_DISABLE_WGET:-}"
#VARSE_DISABLE_IPFS="${VARSE_DISABLE_IPFS:-}"
VARSE_DISABLE_CURL="${VARSE_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$VARSE_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$VARSE_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$VARSE_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Varsechain parameters!
...
main() {
    ...
    cat <<EOF
VARSE - fetch-params.sh

This script will fetch the Varsechain SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Varsechain SNARK parameters. Note that it is
