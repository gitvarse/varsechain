...
const char * const BITCOIN_CONF_FILENAME = "varsechain.conf";
const char * const BITCOIN_PID_FILENAME = "varsechaind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "varsechaind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VarsechainCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VarsechainCore
    // Mac: ~/Library/Application Support/VarsechainCore
    // Unix: ~/.varsechaincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VarsechainCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VarsechainCore";
#else
    // Unix
    return pathRet / ".varsechaincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty varsechain.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Varsechain Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Varsechain Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Varsechain Core developers";
    }
